...
exports.validateTestContext = function validateTestContext (context) {
  ...
  assert.strictEqual('function', typeof context.test.test)

  assert(['selenium', 'puppeteer'].includes(context.test.options.driver), 'Driver must be `selenium` or `puppeteer`')
  if (context.test.options.driver === 'puppeteer') {
    assert.strictEqual('chrome', context.test.options.client.browser.name)
  }
}

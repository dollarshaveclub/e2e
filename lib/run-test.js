...
module.exports = class RunTest {
  constructor (context, { attempt }, eventsEmitter) {
  ...
  createSauceLabsDriver () {
    ...
    return this.runStep({
      ...
      fn: async ({ log }) => {
        ...
        assert.strictEqual(typeof sauceName, 'string')
        const capabilities = {
          name: sauceName,
    ...
    return this.runStep({
      ...
      fn: async () => {
        ...
        try {
          ...
          datadog.sendMetric('quitting_driver.failure', 1)
  ...
  createStep (type = 'step') {
    ...
    const step = (name, fn, _options) => {
      assert.strictEqual(typeof name, 'string', `First argument to ${type} must be the name of the step.`)

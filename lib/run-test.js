...
module.exports = class RunTest {
  ...
  async exec () {
    ...
    console.log(`${context.test.filename}:`)
    console.log(`  config:`)
    Object.keys(logContext).forEach((key) => {
      console.log(`    ${key}: ${logContext[key]}`)
    })
    ...
    return {
      ...
      context,
      results,
      success: results.every(result => result.skip || result.success),
    ...
    if (mode === 'sauce') {
      const s0 = await this.runStep({
        ...
        fn: async () => {
          ...
          console.log(`saucelabs link: https://saucelabs.com/beta/tests/${context.sauce.session_id}/commands`)
        },
  ...
  async runStep ({ name, fn, skip, timeout }) {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log(`    ${result.success ? chalk.green('✔') : chalk.red('✕')} ${name} ${time}`)
      }
      debug(`${key} — finished`)
    }
  }

...
module.exports = class RunTests {
  constructor (tests, _options) {
    ...
    options.stream = options.concurrency === 1 && (options.localConcurrency || options.sauceConcurrency)
  ...
  async runTests () {
    ...
    const results = await Promise.all([
      pAll(this.localTestFns, {
        concurrency: Math.min(options.localConcurrency, 1),
      }),
      pAll(this.sauceTestFns, {
        concurrency: Math.min(options.sauceConcurrency, 1),
      }),
    ])
    ...
    return tests
      .map((test) => {
        ...
        options.clients = options.clients.map((_client) => {
          ...
          return client
        }).filter(client => this.filterClient(client, test))
  ...
  setupTests () {
    ...
    this.getTestContexts().forEach((context) => {
      const runner = new RunTest(context, context.mode)
      runners.push(runner)

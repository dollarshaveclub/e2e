...
module.exports = class RunTests {
  ...
  setupTests () {
    ...
    const allTests = tests
      ...
      .map(test => Object.assign({}, constants.TEST_OPTIONS_DEFAULTS, test))
      // unwind clients
      .map(test => unwind(test, {
        clients: 'client',
      }))
      // add client defaults
      .map(test => {
        const client = Object.assign({}, test.client)
        client.browser = lookupBrowser(client.browser)
        client.platform = Object.assign({}, constants.TEST_OPTIONS_PLATFORM_DEFAULTS, client.platform || {})
    ...
    allTests.forEach((test) => {
      const mode = this.getTestMode(test.client)
      const context = {
        test,
        options,
        env,
      ...
      async function testFn () {
        ...
        const result = await runTest(context, mode)
